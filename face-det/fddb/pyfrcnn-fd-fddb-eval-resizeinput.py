...
save_dir = 'pyfrcn_vgg16_fddb_rlt_resize'
...
def main():
    ...
    for line in fp:
        ...
        msg = 'original size (W x H): {:d} x {:d} '.format(
        ...
        if img.shape[1] > img.shape[0]:
            scale_factor = RESIZED_MIN_SIDE / float(img.shape[0])
        else:
            scale_factor = RESIZED_MIN_SIDE / float(img.shape[1])

        scale_factor_inv = 1.0 / scale_factor

        resized_ht = int(img.shape[0] * scale_factor)
        resized_wd = int(img.shape[1] * scale_factor)

        img = cv2.resize(img, (resized_wd, resized_ht))

        msg = 'resized size (W x H): {:d} x {:d} '.format(
            img.shape[1], img.shape[0])
        print(msg)
        fp_rlt.write(msg + '\n')
        ...
        cls_boxes = boxes[:, 4 * cls_ind:4 * (cls_ind + 1)] * scale_factor_inv
