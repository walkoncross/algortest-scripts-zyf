...
def main():
    ...
    for line in fp:
        ...
        if im.shape[1] > im.shape[0]:
            scale_factor = RESIZED_MIN_SIDE / float(im.shape[0])
        else:
            scale_factor = RESIZED_MIN_SIDE / float(im.shape[1])

        scale_factor_inv = 1.0 / scale_factor

        resized_ht = int(im.shape[0] * scale_factor)
        resized_wd = int(im.shape[1] * scale_factor)

        im = cv2.resize(im, (resized_wd, resized_ht))
